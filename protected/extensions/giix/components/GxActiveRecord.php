...
abstract class GxActiveRecord extends CActiveRecord {
	...
	public function findAllAttributes($attributes = null, $withPk = false, $condition='', $params=array()) {
		...
		if ($attributes === null){
                        if(!is_array($this->representingColumn())){
                                if(!array_key_exists($this->representingColumn(), self::model(get_class($this))->getTableSchema()->columns))
                                        return parent::findAll($criteria);
                        }else{
                                foreach ($this->representingColumn() as $attribute){
                                        if(!array_key_exists($attribute, self::model(get_class($this))->getTableSchema()->columns))
                                                return parent::findAll($criteria);
                                }
                        }
